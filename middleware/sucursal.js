...
proxySucursal.use(async (req, res, next) => {
    try {
      const data = plainToClass(SucursalDTO, req.body, { excludeExtraneousValues: true });
...
middlewareVerify.use((req,res,next) => {
    ...
    let Clone = JSON.stringify(classToPlain(plainToClass(DTO("sucursal").class, expectedPayload,{}, { ignoreDecorators: true })));
    console.log(Clone);
