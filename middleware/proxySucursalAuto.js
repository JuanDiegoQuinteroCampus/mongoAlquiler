...
const proxySucursalAuto = express();


proxySucursalAuto.use(async (req, res, next) => {
    try {
      const data = plainToClass(SucursalAutoDTO, req.body, { excludeExtraneousValues: true });
...
middlewareVerify.use((req,res,next) => {
    ...
    payload = newPayload;
    console.log(payload);
    let Clone = JSON.stringify(classToPlain(plainToClass(DTO("sucursalautomovil").class, {}, { ignoreDecorators: true })));
    console.log(Clone);
